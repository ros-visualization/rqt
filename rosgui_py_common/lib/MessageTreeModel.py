...
class MessageTreeModel(QStandardItemModel):
    ...
    def _get_data_row_for_path(self, slot_name, slot_type, slot_path):
        return (slot_name, slot_type, slot_path)


    def _recursive_create_items(self, parent, slot, slot_name, slot_type, slot_path):
        row = []
        for column in self._get_data_row_for_path(slot_name, slot_type, slot_path):
        ...
        if hasattr(slot, '__slots__') and hasattr(slot, '_slot_types'):
            for child_slot_name, child_slot_type in zip(slot.__slots__, slot._slot_types):
                ...
                self._recursive_create_items(row[0], getattr(slot, child_slot_name), child_slot_name, child_slot_type, child_slot_path)
            ...
            for index, slot in enumerate(slot):
                ...
                child_slot_path = slot_path + '/' + child_slot_name
                self._recursive_create_widget_items(row[0], slot, child_slot_name, child_slot_type, child_slot_path)
